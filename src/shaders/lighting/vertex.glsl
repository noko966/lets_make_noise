...
void main()
{
    ...
    vec3 p = vec3(uv, 1.0) * 2.0;
    float noised = snoise(vec3(p.x, p.y + uTime, p.z));

    float circle = circle(uv, 0.6);
    float h = 20.0;
    float c = 0.5;
    float w = 2.0;

    float height = h * pow( abs( cos( uv.x - 2.0) ), w ) * noised;

    // noised += dist;

    // vec3 pos = position + noised * normal;
